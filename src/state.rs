...
impl StateOfTheArt {
    pub async fn new(surface: Option<&wgpu::Surface<'_>>) -> Self {
        ...
        let texture_views = HashMap::new();
        let samplers = HashMap::new();
        let buffers = HashMap::new();
        let bind_group_layouts = HashMap::new();
        let bind_groups = HashMap::new();
        let shader_modules = HashMap::new();
        let pipeline_layouts = HashMap::new();
        let render_pipelines = HashMap::new();
        let compute_pipelines = HashMap::new();
...
/// Shader Modules
impl StateOfTheArt {
    pub fn add_shader_module_wgsl(&mut self, label: &str, source: &str) {
        ...
        self.shader_modules.insert(label.to_string(), shader_module);
    }
}
...
struct RenderPipelineBuilder<'a> {
    state: &'a mut StateOfTheArt,
    label: &'a str,
}
impl<'a> RenderPipelineBuilder<'a> {
    fn new(state: &'a mut StateOfTheArt, label: &'a str) -> Self {
        RenderPipelineBuilder { state, label }
    }
    ...
    fn build(self) {
        ...
        self.state.device.create_render_pipeline(&desc);
    }
}

/// Render Pipelines
impl StateOfTheArt {
    fn add_render_pipeline<'a>(&'a mut self, label: &'a str) -> RenderPipelineBuilder<'a> {
...
#[test]
fn state_test() {
    let mut state = crate::util::insync(StateOfTheArt::new(None));

    state.add_shader_module_wgsl("sdf", include_str!("sdf_shader.wgsl"));
}
